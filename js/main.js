(function () {
  ...
  function initTelegramSheet() {
    var cfg = window.SITE_CONFIG || {};
    var webUrl = cfg.telegramWeb || 'https://t.me/+79504398338';
    var appUrl = cfg.telegramApp || 'tg://resolve?phone=79504398338';
    var sheet = document.getElementById('tgSheet');
    var appLink = document.getElementById('tgSheetApp');
    var webLink = document.getElementById('tgSheetWeb');
    var cancelBtn = document.getElementById('tgSheetCancel');
    var backdrop = sheet && sheet.querySelector('.tg-sheet__backdrop');

    if (!sheet || !appLink || !webLink) return;
    ...
    function closeSheet() {
      sheet.hidden = true;
      document.body.classList.remove('tg-sheet-open');
    }

    function openSheet() {
      sheet.hidden = false;
      document.body.classList.add('tg-sheet-open');
    }

    document.querySelectorAll('.js-tg-link').forEach(function (link) {
    ...
    appLink.addEventListener('click', function () { closeSheet(); });
    webLink.addEventListener('click', function () { closeSheet(); });
